...
BASE_REPO_URL = 'https://github.com/deepinsight/awone/releases/download/v0.7'

def download(sub_dir, name, force=False, root='~/.awone'):
...
def ensure_available(sub_dir, name, root='~/.awone'):
    return download(sub_dir, name, force=False, root=root)

def download_onnx(sub_dir, model_file, force=False, root='~/.awone', download_zip=False):
    _root = os.path.expanduser(root)
